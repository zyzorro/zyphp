...
void Runner::load_cmd_history()
{
    ...
    run_history = new QFile(QDir::tempPath() + "/appsettings-runner.log");
    // create it if doesn't exists
    if(!run_history->exists())
    QProcess::startDetached(QString("/bin/touch").append(" ").append(QDir::tempPath() + "/appsettings-runner.log"));

    if (!run_history->open(QIODevice::ReadOnly))
    {
        qCritical() << "Error on opening <appsettings-runner.log> file";
        return;
    }
