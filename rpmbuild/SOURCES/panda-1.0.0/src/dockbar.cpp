...
Dockbar::Dockbar(Panda *a, QWidget *parent) : QLabel(parent)
{
    app = a;
...
void Dockbar::read_settings()
{
    ...
    QString stl_name = settings->value("name").toString();
    QString stl_path = settings->value("path").toString();
...
void Dockbar::set_dockapp()
{
    ...
    settings->beginGroup("Dockbar");
    settings->beginGroup("App");

    for (int i = 0; i < settings->childGroups().size(); i++)
    {
        settings->beginGroup(settings->childGroups().value(i)); // App name

        QString name = settings->value("name").toString();
        QString exec = settings->value("exec").toString();
        QString pix = settings->value("pix").toString();
        ...
        settings->endGroup(); // App name
    }
    settings->endGroup(); //App
    settings->endGroup(); //Dockapp
}
...
void Dockbar::create_dock_app(const QString &name, const QString &exec, QWidget *parent)
{
    ...
    settings->setValue("pix", pix);
    settings->endGroup(); //name
    settings->endGroup(); //App
    settings->endGroup(); //Dockbar
