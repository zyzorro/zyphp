...
void Categorymenu::update_menu()
{
    ...
    {
        cat_iter.next();
        appsettings->beginGroup(cat_iter.key()); // category name
        for (int i = 0; i < appsettings->childGroups().size(); i++)
        {
            ...
            QString icon = appsettings->value("pix").toString();
    ...
    appsettings->beginGroup("Launcher-Desk");
    appsettings->beginGroup("Category");
    ...
    {
        cat_desk_iter.next();
        appsettings->beginGroup(cat_desk_iter.key()); // category name
        for (int i = 0; i < appsettings->childGroups().size(); i++)
        {
            appsettings->beginGroup(appsettings->childGroups().value(i)); // App name
            QString name = appsettings->value("name").toString();
    ...
    appsettings->endGroup(); // Category
    appsettings->endGroup(); // Launcher-Desk
}
...
{
    appsettings->remove("Launcher-Desk"); // clear Launcher-Desk entry
    ...
    while (desktop_iter.hasNext())
    {
        ...
        if (! exec.isEmpty() && ! categories.isEmpty() && ! name.isEmpty() && ! icon_name.isEmpty())
        {
            ...
            appsettings->beginGroup("Launcher-Desk");
            appsettings->beginGroup("Category");
            appsettings->beginGroup(categories);
            appsettings->beginGroup(app_name + args); // for i.e. soffice-writer

            appsettings->setValue("name", name);
            appsettings->setValue("exec", full_path);
            appsettings->setValue("pix", icon_path);
...
{
    QSettings *appsettings = new QSettings(QCoreApplication::applicationDirPath() + "/panda.cfg", QSettings::IniFormat);
    appsettings->beginGroup("Style");
    QString stl_name = appsettings->value("name").toString();
    QString stl_path = appsettings->value("path").toString();
    appsettings->endGroup(); //Style
