...
void Deskfolder::mouseReleaseEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
    releaseMouse();
    // save new deskfolder position on desk writing on panda.cfg
    appsettings->beginGroup("Desktop");
    appsettings->beginGroup("Folder");
    appsettings->beginGroup(dir_name); // Icon name
...
void Deskfolder::mouseDoubleClickEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        appsettings->beginGroup("Desktop");
        appsettings->beginGroup("Folder");
        appsettings->beginGroup(dir_name); // Folder name
        ...
        else // open with Filedialog
        {
            ...
            if (file_dialog->exec() == QDialog::Rejected) // on close button
            {
                ...
                appsettings->beginGroup("Desktop");
                appsettings->beginGroup("Folder");
                appsettings->beginGroup(dir_name);
                appsettings->setValue("geometry", file_dialog->geometry());
...
void Deskfolder::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (act != 0 && act != delete_folder)
    {
        ...
        appsettings->beginGroup("Folder");
        appsettings->beginGroup(dir_name); // Folder name
        appsettings->setValue("exec", app_path); // set the favorite application path
        appsettings->endGroup(); // Folder name
        appsettings->endGroup(); // Folder
        appsettings->endGroup(); // Desktop
    }
