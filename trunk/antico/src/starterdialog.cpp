...
void StarterDialog::init()
{
    ...
    setLayout(layout);
    QLabel *name = new QLabel(tr("StarterDialog"), this);
...
void StarterDialog::load_cmd_history()
{
    ...
    QProcess::startDetached(QString("/bin/touch").append(" ").append(QDir::tempPath() + "/antico-StarterDialog.log"));

    if (!run_history->open(QIODevice::ReadOnly))
    {
        qCritical() << "Error on opening <antico-StarterDialog.log> file";
        return;
    }
