...
void Deskfile::mouseReleaseEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
    releaseMouse();
    // save new deskfile position on desk writing on panda.cfg
    appsettings->beginGroup("Desktop");
    appsettings->beginGroup("File");
    appsettings->beginGroup(file_name); // File name
...
void Deskfile::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (act != 0 && act != delete_file)
    {
        ...
        appsettings->setValue("pix", icon); // replace the default pixmap
        appsettings->endGroup(); // File name
        appsettings->endGroup(); // File
        appsettings->endGroup(); // Desktop
    }
