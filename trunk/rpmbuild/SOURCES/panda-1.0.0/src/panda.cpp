...
Panda::Panda(int &argc, char **argv) : QApplication(argc, argv)
{
    set_event_names();
...
void Panda::send_supported_hints()
{
...
bool Panda::x11EventFilter(XEvent *event)
{
    Frame *frm;
...
void Panda::create_frame(Window c_win, Dockbar *dock, Desk *desk) // create new frame around the client app
{
...
bool Panda::check_net_sys_tray_for(Window c_win) const
{
    Atom type_ret = None;
...
void Panda::check_window_type(Window c_win) // chech the window type before mapping
{
    Atom type_ret = None;
    Atom *win_type;
    unsigned char *data = 0;
...
void Panda::check_wm_transient_for(Window c_win)
{
    Atom type_ret = None;
...
void Panda::print_window_prop(Window c_win) // print the window properties
{
    int num_prop_ret = 0;
...
void Panda::raise_next_frame() // raise next frame on [Alt+Tab] key combination
{
...
void Panda::set_active_frame(Frame *frm) // activation of selected frame and unactivation of others
{
...
void Panda::send_configurenotify(Frame *frm)
{
    XConfigureEvent ce;
...
void Panda::wm_refresh()
{
    qDebug() << "Refreshing Panda WM ...";
...
void Panda::wm_quit()
{
    ...
    if (ret == QDialog::Accepted)
    {
        ...
        dsk->close();
        XSync(QX11Info::display(), False);
        QProcess::startDetached(QString("/bin/rm").append(" ").append(QDir::tempPath() + "/panda-runner.log"));
        qDebug() << "Quit Panda WM ...";
        XCloseDisplay(QX11Info::display());
        emit lastWindowClosed();
    }
}

void Panda::wm_shutdown()
{
    Msgbox msg;
...
void Panda::run_app_at_startup()
{
    // default path
    settings = new QSettings(QCoreApplication::applicationDirPath() + "/panda.cfg", QSettings::IniFormat, this);
    ...
    {
        QStringList args;
        QString app_name = settings->childGroups().value(i);
        settings->beginGroup(app_name); // App group
        QString name = settings->value("name").toString();
        QString path = settings->value("path").toString();
        args  << settings->value("args").toString();
...
Filedialog * Panda::get_file_dialog()
{
    return file_dialog;
}

Desk * Panda::get_desktop()
{
    return dsk;
}

Dockbar * Panda::get_dockbar()
{
    return dock;
}

Categorymenu * Panda::get_category_menu()
{
    return cat_menu;
}

void Panda::set_settings()
{
    // default path
    settings = new QSettings(QCoreApplication::applicationDirPath() + "/panda.cfg", QSettings::IniFormat, this);
    // set default style on first installation, if no "/panda.cfg" is set
    if (settings->childGroups().isEmpty())
    {
        qDebug() << "Set default settings ...";
        settings->beginGroup("Style");
        settings->setValue("name", "default.stl");
        settings->setValue("path", QCoreApplication::applicationDirPath() + "/theme/default/");
        settings->endGroup(); //Style
        settings->sync();
    }
    if (settings->status() == QSettings::AccessError)
    {
        qFatal ("Error on setting panda.cfg");
    }
    ...
    if (style->childGroups().isEmpty())
    {
        ...
        style->setValue("launcher_pix", "panda.png");
