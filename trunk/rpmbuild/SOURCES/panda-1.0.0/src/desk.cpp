...
Desk::Desk(Panda *a, QWidget *parent) : QLabel(parent)
{
    app = a;
...
void Desk::set_desk_icons()
{
    ...
    appsettings->beginGroup("Desktop");
    appsettings->beginGroup("Folder");

    for (int i = 0; i < appsettings->childGroups().size(); i++)
    {
        appsettings->beginGroup(appsettings->childGroups().value(i)); // Folder name

        QString name = appsettings->value("name").toString();
        QString path = appsettings->value("path").toString();
        QRect geom = appsettings->value("geometry").value<QRect>();
        QPoint pos = appsettings->value("pos").value<QPoint>();
        ...
        appsettings->endGroup(); // Folder name
    }
    appsettings->endGroup(); //Folder

    // read deskfile name, path, pix, pos and restore on desktop
    appsettings->beginGroup("File");

    for (int i = 0; i < appsettings->childGroups().size(); i++)
    {
        appsettings->beginGroup(appsettings->childGroups().value(i)); // File name

        QString name = appsettings->value("name").toString();
        QString path = appsettings->value("path").toString();
        QString pix = appsettings->value("pix").toString();
        QPoint pos = appsettings->value("pos").value<QPoint>();
        ...
        appsettings->endGroup(); // File name
    }
    appsettings->endGroup(); //File
    ...
    for (int i = 0; i < appsettings->childGroups().size(); i++)
    {
        appsettings->beginGroup(appsettings->childGroups().value(i)); // App name

        QString name = appsettings->value("name").toString();
        QString exec = appsettings->value("exec").toString();
        QString pix = appsettings->value("pix").toString();
        QPoint pos = appsettings->value("pos").value<QPoint>();
    ...
    appsettings->endGroup(); //Desktop

    // read Starter/ender and restore on desktop


    appsettings->beginGroup("Starter");
    QPoint starterpos = appsettings->value("pos").value<QPoint>();
    appsettings->endGroup(); //Starter

    appsettings->beginGroup("Ender");
    QPoint enderpos = appsettings->value("pos").value<QPoint>();
    appsettings->endGroup(); //Ender

   starter = new Starter(this);
   starter->move(starterpos);
   ender = new Ender(this);
   ender->move(enderpos);
 //   starter->move(pos);
}
...
void Desk::create_desk_folder(const QString &name, const QString &path, const QRect &geometry, const QPoint &pos, QWidget *parent)
{
    ...
    appsettings->setValue("exec", ""); // for future association with favorite app to open
    appsettings->setValue("pos", pos);
    appsettings->setValue("geometry", geometry);
...
{
    // remove the deskfolder from desk and from panda.cfg
    appsettings->beginGroup("Desktop");
    appsettings->beginGroup("Folder");
    appsettings->remove(d_folder->get_dir_name());
    appsettings->endGroup(); // Folder
    appsettings->endGroup(); // Desktop
...
void Desk::create_desk_file(const QString &name, const QString &path, const QString &icon, const QPoint &pos, QWidget *parent)
{
    ...
    appsettings->setValue("exec", ""); // for future association with favorite app to run
    appsettings->setValue("pos", pos);
    appsettings->endGroup(); //name
    appsettings->endGroup(); //File
    appsettings->endGroup(); //Desktop
...
{
    // remove the deskicon from desk and from panda.cfg
    appsettings->beginGroup("Desktop");
    appsettings->beginGroup("File");
    appsettings->remove(d_file->get_file_name());
    appsettings->endGroup(); // File
    appsettings->endGroup(); // Desktop
...
void Desk::create_desk_app(const QString &name, const QString &exec, const QString &icon, const QPoint &pos, QWidget *parent)
{
    ...
    appsettings->beginGroup(name);
    appsettings->setValue("name", name);
    appsettings->setValue("exec", exec);
    appsettings->setValue("pix", icon);
    appsettings->setValue("pos", pos);
    appsettings->endGroup(); //name
    appsettings->endGroup(); //App
...
void Desk::remove_deskapp(Deskapp *d_app) // remove from "Delete link" right button mouse on Desktop and from rubberband selection
{
    ...
    appsettings->beginGroup("Desktop");
    appsettings->beginGroup("App");
    appsettings->remove(d_app->get_app_name());
    appsettings->endGroup(); // App
    appsettings->endGroup(); // Desktop
