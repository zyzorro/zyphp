...
void Manager::set_run_args(QTableWidgetItem *item) // to set run arguments for startup applications
{
    ...
    appsettings->beginGroup("Startup");
    appsettings->beginGroup(name_item->text()); // application name
    appsettings->setValue("args", item->text()); // application args on startup
    appsettings->endGroup(); // Name
    appsettings->endGroup(); // Startup
}
...
{
    //////////////////////////////////////////////////////////////////
    appsettings->beginGroup("Style"); // save the style
    appsettings->setValue("name", stl_name);
    appsettings->setValue("path", stl_path);
    appsettings->endGroup(); //Style
    //////////////////////////////////////////////////////////////////
    style->beginGroup("Frame");
    style->beginGroup("Border");
...
void Manager::add_app_pressed() // add selected app on lancher menu (in the select Category)
{
    if (tree_view->selectionModel()->currentIndex().isValid())
    {
        ...
        if ( ! app.isEmpty() && info.isExecutable() && ! info.isDir())
        {
            ...
            appsettings->beginGroup("Launcher");
            appsettings->beginGroup("Category");
            appsettings->beginGroup(cat_map.value(category_combo->currentText())); // the select Category type from combobox (with translation)
            appsettings->beginGroup(app);

            appsettings->setValue("name", app);
            appsettings->setValue("exec", path);
            appsettings->setValue("pix", icon);
            appsettings->endGroup(); // App name
            appsettings->endGroup(); // Category type
            appsettings->endGroup(); // Category group
            appsettings->endGroup(); // Launcher
            update_remove_list();
            Msgbox msg;
...
void Manager::add_run_app_pressed() // add selected app on "Run at startup" list
{
    ...
    if (file_dialog->exec() == QDialog::Accepted)
    {
        ...
        if (! name.isEmpty() && pathinfo.isExecutable())
        {
            ...
            appsettings->beginGroup("Startup");
            appsettings->beginGroup(name);
            appsettings->setValue("name", name);
            appsettings->setValue("path", path);
            appsettings->setValue("args", ""); // for future settings
            appsettings->endGroup(); // Name
            appsettings->endGroup(); // Startup
        }
    }
...
void Manager::remove_run_app_pressed() // remove selected app from "Run at startup" list
{
    ...
    if (current_row >= 0) // if some row is selected
    {
        ...
        appsettings->beginGroup("Startup");
        appsettings->remove(name_item->text()); // remove the application
        appsettings->endGroup(); // Startup
...
void Manager::remove_app_pressed() // remove selected app from Launcher menu
{
    if (app_tree->currentItem() != NULL && app_tree->currentItem()->parent() != NULL) // remove only is item is selected (no if it's a Category)
    {
        ...
        appsettings->beginGroup("Launcher");
        appsettings->beginGroup("Category");
        appsettings->beginGroup(cat_group); // Category group (parent of application)
        qDebug() << "Parent Application:" << cat_group;
        appsettings->remove(app); // remove the application
        appsettings->endGroup(); // Category group
        appsettings->endGroup(); // Category
        appsettings->endGroup(); // Launcher
        update_remove_list();
        Msgbox msg;
...
void Manager::update_remove_list() // update the Category/Apps list on remove tab
{
    ...
    for (int c = 0; c < appsettings->childGroups().size(); c++)
    {
        QString cat_name = appsettings->childGroups().value(c); //Category group (Utility, Office ...)
        appsettings->beginGroup(cat_name); // Category group
        ...
        for (int i = 0; i < appsettings->childGroups().size(); i++)
        {
            QString app_name = appsettings->childGroups().value(i); //Application name
...
void Manager::update_run_table() // update the "Run at startup" list
{
    ...
    for (int i = 0; i < appsettings->childGroups().size(); i++)
    {
        QString app_name = appsettings->childGroups().value(i);
        appsettings->beginGroup(app_name); // App group
        QString path = appsettings->value("path").toString(); // App path
        QString args = appsettings->value("args").toString(); // App arguments
        Appicon app_icon; // get application icon
        QString icon = app_icon.get_app_icon(app_name);
